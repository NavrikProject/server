                ...
                (err, result) => {
                  ...
                  if (result.recordset.length > 0) {
                    ...
                    sql.connect(config, (err) => {
                      ...
                      request.query(
                        "UPDATE trainer_profile SET trainer_firstname=@firstName, trainer_lastname=@lastName,trainer_mobile=@mobileNumber,trainer_lnprofile=@lnProfile,trainer_qualifications=@qualification,trainer_skills=@skills,trainer_exp_yrs=@experience,trainer_pref_time=@prefTime,trainer_engment_typ=@engType,trainer_no_of_hrs_daily=@noOfHrs,trainer_location=@location,trainer_description=@description, WHERE trainer_id = @trainerId",
                        (err, result) => {
                          if (err) return res.send({ error: err.message });
                          if (result) {
                            sgMail.setApiKey(process.env.SENDGRID_API_KEY);
